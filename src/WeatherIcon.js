...
const WeatherIcon = ({ currentWeatherCode, moment }) => {
  ...
  useEffect(() => {
    function weatherCode2Type(currentWeatherCode) {
      let currentWeatherType = "";
      for (let index in weatherCode) {
        if (!weatherCode[`${index}`].includes(Number(currentWeatherCode)))
          continue;
        currentWeatherType = index;
        break;
      }
      return currentWeatherType;
    }
    const currentWeatherType =
      weatherCode2Type(currentWeatherCode) || "沒有東西";
    setCurrentWeatherIcon(currentWeatherType);
  }, [currentWeatherCode, moment]);

  return (
    <IconContainer>{weaterIcon[moment][currentWeatherIcon]}</IconContainer>
  );
};
